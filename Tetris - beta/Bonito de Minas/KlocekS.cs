...
namespace Bonito_de_Minas
{
    internal class KlocekS : Klocek
    {
        ...
        public KlocekS()
        {
            ...
            look = 4;
            SMap = new Byte[3, 3];
            lastSMap = new Byte[3, 3];
            SMap[0, 2] = look; SMap[0, 1] = look; SMap[1, 1] = look; SMap[1, 0] = look;
            //for (int i = 0; i < 3; i++)
            //    for (int j = 0; j < 3; j++)
            //        lastSMap = SMap;
        }
        public KlocekS(Board map, int x, int y)
        {
            MainBoard = map;
            height = 2;
            this.x = x; this.y = y;
        ...
        public override void MoveKlocki()
        {
            if (checkdownside())
            {
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 3; j++)
                        tmp[i, j] = SMap[i, j];
                if (MainBoard.DoesItFit(tmp, lastSMap, look, x, y, false, 0))
                {
                    fillklocekinmap(2);
                    x++;
                }
            }
            else if (MainBoard.DoesItFit(SMap, lastSMap, look, x, y, false, 0))
            {
                height = 3;
                fillklocekinmap(3);
                x++;
        ...
        public override Byte[,] Rotate(int direction)
        {
            ...
            if (MainBoard.DoesItFit(tmp, SMap, look, x, y, true, 0))
            {
                SMap = tmp;
            }
            return tmp;
        ...
        public override bool ControlKlocek(int direction)
        {
            ...
            if (direction == 1)
            {
                ...
                MainBoard.DoesItFit(SMap, lastSMap, look, x, y, false, 0);
                if (MainBoard.DoesItFit(SMap, lastSMap, look, x, y-1, false, 0))
                {
                    y--;
                }
            }
        ...
        public override void clearside(bool side)
        {
            ...
            {
                for (int i = 0; i < 3; i++)
                    if (MainBoard.Map[x+i, y - 1] == look && SMap[i, 2] == look)
                        MainBoard.Map[x + i, y - 1] = 0;
            }
            else //right
            {
                for (int i = 0; i < 3; i++)
                    if (MainBoard.Map[x + i, y + 3] == look && SMap[i, 0] == look)
        ...
        bool czyspadl(int sizeofheight)
        {
            ...
            if (x + sizeofheight - 1 == MainBoard.rows)
                return true;
            for (int i = 0; i < 3; i++)
                if (MainBoard.Map[x + sizeofheight - 1, y + i] != 0 && SMap[sizeofheight - 1, i] != 0)
                    return true;

            for (int i = 0; i < sizeofheight; i++)
                for (int j = 0; j < 3; j++)
                {
                    if (MainBoard.Map[x + i, y + j] == look && SMap[i, j] == look)
                    {
            ...
            for (int i = 0; i < sizeofheight; i++)
                for (int j = 0; j < 3; j++)
                {
                    if (MainBoard.Map[x + i, y + j] != 99 && MainBoard.Map[x + i, y + j] != 0 && SMap[i, j] != 0)
                    {
